from DataStructure.bst_del import min_right_subtree
...
class BSTDemo:
    ...
    def _insert(self, curr, key):
        if key.data >= curr.scheduled_end:
            if curr.right_child == None:
                curr.right_child = key
                self.helpful_print(key, True)
            else:
                self._insert(curr.right_child, key)
        elif key.scheduled_end <= curr.data:
    ...
    def _find_val(self, curr, key):
        if curr:
            if key == curr.data:
                ...
                self._find_val(curr.right, key)
        return f"{key} not found in tree"
        ...
        if curr:
            if key == curr.data:
                if curr.left_child == None and curr.right_child == None:
                    ...
                    if prev:
                        min_child = self.min_right_subtree(self, curr.right_child)
                        curr.data = min_child.data
                        self.delete_val(curr.right_child, curr, False, min_child.data)
                    else:
                        self.root.data = curr.left_child.data
...
super_scheduler = BSTDemo()
super_scheduler.insert('12:10,20,meeting')
super_scheduler.insert('12:15,20,meeting')
super_scheduler.insert('15:15,50,conference')
super_scheduler.in_order()